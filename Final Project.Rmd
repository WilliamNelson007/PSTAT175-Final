...
gracer1.1 = gracer1.1 %>% mutate(start=ifelse(row%%2==1,0,revascdays),stop=ifelse(row%%2==1,revascdays,days),status=ifelse(row%%2==1,0,1)) %>% select(!row)
#remove the duplicate rows with start=stop=0 (or revasc=0)
gracer1.2 <-gracer1.1 %>% filter(start!=stop)
...
```{r}
tvc.1 <- coxph(Surv(start, stop, status) ~ revasc, data = grace2)
summary(tvc.1)
```
