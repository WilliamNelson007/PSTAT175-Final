...
```{r}
# WE DON'T INCLUDE INTERVAL IN THIS ALLMOD BECAUSE IT DEPENDS ON TIME
# NO STRATIFYING OR TESTING FOR THE PH ASSUMPTION 
# SHOULD WE FOLLOW THE ORDER WE DETERMINED FROM THE PREVIOUS PART?

#allmod2 <- coxph(Surv(start,stop,status) ~ revasc + age + sysbp + stchange, data = grace2)

allmod2 <- coxph(Surv(start,stop,status) ~ age + sysbp + revasc + stchange, data = grace2)

summary(allmod2)
```

From the summary output of our model we can still see all our covariates have significant effects of the on the time till an event, with p-values much smaller than our significance level of 0.05. Our overall likelihood ratio test also reports an extremely small p-value which tells us that our model is working very well and the covariates are providing the necessary information to estimate time till an event. We can also look at the anova output to check the significance of each covariate individually as they are added:

```{r}
anova(allmod2)
```

Similar to the summary output, we see that all four covariates have statistically significant p-values for their Chi-Squared log-likelihood test, which tells us that each of the four covariates provide significant information to the model that allow it to better model the time to an event. Because we still want to analyze the effect of revascularization, we can calculate a 95% confidence interval for the hazard ratio between the treatment and control groups:

```{r}
# hazard ratio for revasc
exp(coef(allmod2)[3] + c(-1.96, 1.96) * sqrt(allmod2$var[3, 3]))
```

The time-varying model still indicates that revascularization is a highly significant predictor of survival (p \< 0.0001), with a 95% confidence interval of the hazard ratio being [0.4892116, 0.7787804]. The hazard ratio being less than 1 signifies a protective effect of revascularization.

To ensure that the model of best fit we found still works properly, we want to make sure that our covariates do not have a significant interaction with the interval variable which would tell us that the covariate also varies with time. We will fit models with an interaction term between the interval signifier and each covariate, and analyze the results of these models:

#### \^\^\^ Check reasoning/wording

```{r}
# Interactions between interval and other covariates besides revasc
ageInteractMod <- coxph(Surv(start, stop, status) ~ interval * age, data = grace2)
summary(ageInteractMod)# Significant interaction
```

We see in this summary output that the age interaction model shows a significant interaction between interval and age because the p-value of 0.04 is just below our significance threshold so we conclude that there is a significant interaction between the interval and age measurement. This tells us that there is a statistically significant difference between the effect of the age covariate on the time to an event between the pre and post-revascularization time intervals, and this suggests that the age covariate may have some underlying time correlation.

```{r}
sysbpInteractMod <- coxph(Surv(start, stop, status) ~ interval * sysbp, data = grace2)
summary(sysbpInteractMod) # Not significant interaction
```

The sysbp interaction model does not show a significant interaction between interval and sysbp with a p-value above our significance threshold of 0.05. We can conclude that there is no significant interaction between the patients systolic blood pressure measurement and the interval signifier.

```{r}
stchangeInteractMod <- coxph(Surv(start, stop, status) ~ interval * stchange, data = grace2)
summary(stchangeInteractMod) # Not significant interaction
```

The stchange interaction model does not show a significant interaction between interval and stchange with a p-value of 0.835. This p-value is large and leads us to conclude that there is no interaction effect between the stchange classifier and the interval signifier.

### -- WE PROBABLY NEED TO DELETE ALL OF THIS BELOW (NO TESTING FOR PH ASSUMPTION IN THIS PART) --

### Check Cox PH Model Assumptions: Log-Log Plot

```{r}
ggsurvplot(survfit(Surv(start,stop,status) ~ interval, data = grace2),
           fun = "cloglog") + 
  labs(x = "log(Days)", y = "Complementary Log-Log",
       title = "Log-Log Plot by Interval")
```

The Proportional Hazards assumption appears to be violated in this model with just the interval as a covariate because we see the lines intersect rather than stay parallel to each other.
...
# Suggested:
intervalMod <- coxph(Surv(start, stop, status) ~ interval, data = grace2)
summary(intervalMod)
anova(intervalMod)
# Significant result with interval as covariate representing treatment (interval = 2)
# and control (interval = 1)
...
# Stratify?
strataMod <- coxph(Surv(start, stop, status) ~ strata(interval) + age + revasc + sysbp + stchange, data = grace2)
summary(strataMod)
anova(strataMod)
```
